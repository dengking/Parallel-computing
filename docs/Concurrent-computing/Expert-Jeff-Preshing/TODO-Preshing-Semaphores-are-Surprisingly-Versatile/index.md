...
I’ve published the implementation as [`AutoResetEvent`](https://github.com/preshing/cpp11-on-multicore/blob/master/common/autoresetevent.h). This time, the box office has a different way to keep track of how many threads have been sent to wait in the queue. When `m_status` is negative, its magnitude indicates how many threads are waiting:
